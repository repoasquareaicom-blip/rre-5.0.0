...
{
    public partial class HsnStockReport : Form
    {
        static string Conn = ConfigurationManager.ConnectionStrings["con"].ConnectionString;
        QuotationBal objQuotationbal = new QuotationBal();
        public HsnStockReport()
        {
            InitializeComponent();
            Bindcompany();
        }

        private void dgvStockrpt_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        public void searchGSTWithDate()
        {
            ...
            using (SqlConnection con = new SqlConnection(Conn))
            {
                ...
                cmd.Parameters.AddWithValue("@HSN", HsnData);
               
                cmd.CommandType = CommandType.StoredProcedure;

                if (CompanyName == "R.R. ELECTRICAL AGENCIES")
                {
                    cmd.CommandText = "GetHSNReport_Electricals";
                }
                if (CompanyName == "R.R. PIPES")
                {
                    cmd.CommandText = "GetHSNReport_Pipes";
                }
                cmd.Connection = con;
                SqlDataAdapter ad = new SqlDataAdapter(cmd);
                ad.Fill(dt);
