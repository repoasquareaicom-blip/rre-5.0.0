...
{
    public partial class ReUpload : Form
    {
        public ReUpload()
        {
            InitializeComponent();
        ...
        {
            DataTable filterDatas = sheetData;
            //Validate Fields
            if (ValidateUploadFields(filterDatas) == true)
            {
                string SortByVal = string.Empty;
                SortByVal = "Stock";
                string SortOrderVal = "asc";
                filterDatas.DefaultView.Sort = SortByVal + " " + SortOrderVal;
                filterDatas.Columns.Add("Comments");

                filterDatas.Columns.Add("ModifiedStock");
                filterDatas.Columns.Add("UploadedBy");
                filterDatas.Columns.Add("UploadedOn");
                filterDatas.Columns.Add("IsModified");
                filterDatas.Columns.Add("ModifiedBy");
                filterDatas.Columns.Add("ModifiedOn");




                bool validation = true;
                foreach (DataRow r in filterDatas.Rows)
                {
                    string Check = "";
                ...
                dataGridView1.DataSource = filterDatas;
                ...
                DataTable dt = filterDatas;
                dataGridView1.DataSource = filterDatas;
                dataGridView1.Columns["ItemName"].Width = 300;
                dataGridView1.Columns["Rack"].Width = 300;
                //dataGridView1.Columns["Comments"].Visible = false;
                dataGridView1.Columns["ModifiedStock"].Visible = false;
                dataGridView1.Columns["UploadedBy"].Visible = false;
                dataGridView1.Columns["UploadedOn"].Visible = false;
                dataGridView1.Columns["IsModified"].Visible = false;
                dataGridView1.Columns["ModifiedBy"].Visible = false;
                dataGridView1.Columns["ModifiedOn"].Visible = false;
                    ...
                    if (result == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            if (filteredCount <= 0)
                            {
                                ...
                                rsData.Columns.Remove("UploadedBy");
                                rsData.Columns.Remove("UploadedOn");
                                rsData.Columns.Remove("IsModified");
                                rsData.Columns.Remove("ModifiedBy");
                                rsData.Columns.Remove("ModifiedOn");
                                rsData1 = CashEndCloseBAL.StockReuploadFilse(rsData, Convert.ToInt16(UserId));
                                MessageBox.Show(rsData1.Rows[0]["Result"].ToString());
                                rsData.Columns.Add("Comments");

                                rsData.Columns.Add("UploadedBy");
                                rsData.Columns.Add("UploadedOn");
                                rsData.Columns.Add("IsModified");
                                rsData.Columns.Add("ModifiedBy");
                                rsData.Columns.Add("ModifiedOn");
                                textBox1.Text = "";
        ...
        private void ReUpload_Load(object sender, EventArgs e)
        {
            label3.Text = "0";
        ...
        protected void genExcel(DataTable Dt)
        {
            ...
            StreamWriter wr = new StreamWriter(@"D:\\ProductMasterTemplate_Reupload.xls");
