    ...
    class ReciptDal
    {
		...
		public void PrintHeader()
		{
			if (this.dsMain.Tables[0].Rows.Count > 0)
			{
				...
				this.rdr.WriteLine("");
				this.PrintLine();
				this.rdr.Write((char)15 +"RECEIPT NO : " +  this.GetFormatedText(Convert.ToString(this.dsMain.Tables[0].Rows[0]["ReceiptId"]), 20));
				this.rdr.Write("DATE : " + this.GetFormatedText(Convert.ToString(this.dsMain.Tables[0].Rows[0]["Transdate"]), 20));
                this.rdr.WriteLine("" + (char)18);
				this.PrintLine();
				this.rdr.WriteLine("");
                this.rdr.WriteLine(this.GetFormatedText("Received with thanks from M/S.., ", 40));
                this.rdr.WriteLine("");
				this.rdr.WriteLine(this.GetFormatedText(Convert.ToString(this.dsMain.Tables[0].Rows[0]["CustomerName"]), 40));
				this.rdr.WriteLine("");
				this.rdr.WriteLine(this.GetFormatedText("the sum Of Rupees " + Convert.ToString(this.dsMain.Tables[0].Rows[0]["Numberinwords"]), 40));
				this.rdr.WriteLine("");
				if (!string.IsNullOrEmpty(Convert.ToString(this.dsMain.Tables[0].Rows[0]["Mode"])))
				{
					this.rdr.WriteLine(this.GetFormatedText("through by  " + Convert.ToString(this.dsMain.Tables[0].Rows[0]["Mode"]), 40));
					this.rdr.WriteLine("");
				}
				this.rdr.WriteLine(this.GetFormatedText("toward your account.", 40));
				this.rdr.WriteLine("");
				this.PrintLine();
				...
				{
					this.rdr.Write(this.GetFormatedText("BILL DETAILS:" + Convert.ToString(this.dsMain.Tables[0].Rows[0]["EntityId"]), 40));
				}
                this.rdr.WriteLine("");
				this.PrintLine();
				this.rdr.WriteLine("");
				this.rdr.WriteLine("");
				this.rdr.WriteLine("");
				this.rdr.Write(this.GetFormatedText("Signature", 20));
				this.rdr.Write(this.GetRightFormatedText("Authorized Signatory", 20));
				this.rdr.WriteLine("");
				this.rdr.WriteLine("");
				this.rdr.WriteLine("");
				this.rdr.WriteLine("");
