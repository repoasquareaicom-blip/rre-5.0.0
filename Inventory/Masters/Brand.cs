...
namespace Inventory.Masters
{
    public partial class Brand : Form
    {
        ...
        {
            DataTable dt = CategoryBAL.objCategoryDAL.getbrandval();
        ...
        private void InsertUpdateRightToInformation()
        {
            ...
            int Status = CategoryBAL.objCategoryDAL.Savebrand(ObjCategoryBAL);
        ...
        private void dgvuom_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (dgvuom.Columns[e.ColumnIndex].HeaderText == "Edit")
                {
                    ...
                    txtcategory.Text = Convert.ToString(dgvuom.Rows[e.RowIndex].Cells["Brandname"].Value);
                }

                else if (dgvuom.Columns[e.ColumnIndex].HeaderText == "Delete")
                {
                    string namestatus = Convert.ToString(dgvuom.Rows[e.RowIndex].Cells["Brandname"].Value);
        ...
        {
            int result = 0;
            result = CategoryBAL.objCategoryDAL.deletebrand(Convert.ToInt32(lblhidden.Text));
